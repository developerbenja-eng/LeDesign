    ...
    for (let i = 0; i < featureCollection.length; i++) {
      ...
      if (geometry.type === 'LineString' || geometry.type === 'MultiLineString') {
        const lines = geometry.type === 'LineString'
          ? [geometry.coordinates as GeoJSONLineString]
          : (geometry.coordinates as GeoJSONMultiLineString);
        ...
        for (let j = 0; j < lines.length; j++) {
          ...
          features.push({
            type: 'road',
            geometry: { type: 'LineString', coordinates: coords.map(([x, y]) => [x, y] as [number, number]) },
            confidence: 0.9,
            source: 'ide_chile',
            useAsBreakline: true,
  ...
  try {
    ...
    for (let i = 0; i < featureCollection.length; i++) {
      ...
      if (geometry.type === 'Point') {
        const [x, y] = geometry.coordinates as GeoJSONPosition;
        ...
        const rings = geometry.type === 'Polygon'
          ? [(geometry.coordinates as GeoJSONPolygon)[0]]
          : (geometry.coordinates as GeoJSONMultiPolygon).map(poly => poly[0]);
        ...
        for (let j = 0; j < rings.length; j++) {
          ...
          features.push({
            type: 'lake',
            geometry: { type: 'Polygon', coordinates: coords.map(([x, y]) => [x, y] as [number, number]) },
            confidence: 0.9,
            source: 'ide_chile',
            useAsBreakline: false,
            useAsFlatConstraint: true,
