 ...
 */
export const SPEED_TO_SPACING: Record<number, { min: number; optimal: number; max: number }> = {
  20: { min: 60, optimal: 80, max: 100 },
  25: { min: 75, optimal: 100, max: 130 },
  30: { min: 90, optimal: 120, max: 160 },
  35: { min: 110, optimal: 150, max: 200 },
  40: { min: 130, optimal: 180, max: 240 },
};
 ...
 */
export function getRecommendedSpacing(targetSpeed: number): { min: number; optimal: number; max: number } {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (dist < spacing.min) {
      issues.push(
        ...
        `are too close (${dist}m < ${spacing.min}m minimum)`
      );
    }
    ...
    if (dist > spacing.max) {
      issues.push(
        ...
        `may allow speed recovery (${dist}m > ${spacing.max}m maximum)`
      );
    }
  }
