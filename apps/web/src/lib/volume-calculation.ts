...
// Conditional server-only import (skip in test environment)
if (typeof window === 'undefined' && process.env.NODE_ENV !== 'test') {
  try {
    require('server-only');
  } catch (e) {
    // Ignore in non-Next.js environments
  }
}
...
/**
 * Calculate cross sections at regular intervals
 */
...
/**
 * Generate a heat map of cut/fill depths
 */
...
/**
 * Calculate mass haul diagram data
 */
...
export function calculateMassHaul(
  ...
  freeHaulDistance: number = 100 // meters
): MassHaulStation[] {
  ...
  for (let i = 0; i < sections.length - 1; i++) {
    ...
    const area1 =
      (s1.fillDepth || 0) - (s1.cutDepth || 0);
    const area2 =
      (s2.fillDepth || 0) - (s2.cutDepth || 0);
