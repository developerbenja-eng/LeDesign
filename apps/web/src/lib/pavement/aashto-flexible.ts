...
export function calculateStructuralNumber(input: FlexiblePavementInput): number {
  ...
  const logW18 = safeLog(W18, 'W18');
  const logMr = safeLog(subgradeMr, 'subgradeMr');
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    const term3Den = 0.4 + safeDivide(1094, safePow(SN + 1, 5.19, 'SN+1'), '(SN+1)^5.19');
    const term3 = safeDivide(term3Num, term3Den, 'term3Den');
...
export function calculateLayerCoefficient(
  ...
  switch (layerType) {
    case 'asphalt':
      ...
      coefficient = 0.4 * safePow(modulusPsi / 450000, 0.33, 'EAC/450000');
      break;

    case 'base':
      // EBS typically 15,000-40,000 psi
      coefficient = 0.249 * safeLog(modulusPsi, 'EBS') / Math.LN10 - 0.977;
      break;

    case 'subbase':
      // ESB typically 10,000-25,000 psi
      coefficient = 0.227 * safeLog(modulusPsi, 'ESB') / Math.LN10 - 0.839;
      break;
