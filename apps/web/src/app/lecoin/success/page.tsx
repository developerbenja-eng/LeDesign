...
import { CheckCircle2, Coins, Heart, Mail, Download, ArrowRight } from 'lucide-react';
...
function SuccessContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/lecoin/session?session_id=${sessionId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.error) {
          setError(data.error);
  ...
  const coinsIssued = session?.metadata?.coinsToIssue
    ? parseInt(session.metadata.coinsToIssue)
    : 0;
  const amount = session?.amount_total ? session.amount_total / 100 : 0;
  const donorName = session?.metadata?.donorName || 'Friend';
          ...
          <div className="glass-card rounded-xl p-8 mb-8 animate-slide-up">
            ...
            <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/30">
              <p className="text-sm text-slate-300">
                <strong className="text-white">✅ Payment Confirmed</strong>
                <br />
...
export default function LeCoinSuccessPage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center">
